class Game:
    ...
    def to_json(self):
        ...
        game_json['board'] = str(self.board)
        game_json['game_data'] = self.game_data

        start_date = f'{self.start_date:%Y-%m-%d %H:%M:%S%z}' if self.start_date else None
        game_json['start_date'] = start_date

        end_date = f'{self.end_date:%Y-%m-%d %H:%M:%S%z}' if self.end_date else None
